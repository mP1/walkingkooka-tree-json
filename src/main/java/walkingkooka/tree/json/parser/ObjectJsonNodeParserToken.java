 ...
 */
public final class ObjectJsonNodeParserToken extends ParentJsonNodeParserToken<ObjectJsonNodeParserToken> {

    static ObjectJsonNodeParserToken with(final List<ParserToken> value, final String text) {
        return new ObjectJsonNodeParserToken(copyAndCheckTokens(value),
            checkText(text));
    }

    private ObjectJsonNodeParserToken(final List<ParserToken> value, final String text) {
        super(value, text);
        this.checkKeys(text);
    }
    ...
    JsonNode toJsonNodeOrNull() {
        StringJsonNodeParserToken key = null;
        ...
        for (ParserToken element : ParserToken.filterWithoutNoise(this.value)) {
            if (element instanceof JsonNodeParserToken) {
                ...
                if (null == key) {
                    key = j.cast(StringJsonNodeParserToken.class);
                } else {
    ...
    @Override
    public ObjectJsonNodeParserToken setChildren(final List<ParserToken> children) {
        return ParserToken.parentSetChildren(
            this,
            children,
            ObjectJsonNodeParserToken::with
        );
    }
